<?php
    ...
    class Route {
        ...
        public function setPath($path) {
            ...
            if(strlen($path) > 0){
                $pathRegex = preg_replace_callback('/\{\{(|\*|s|i|n):(\w+)\}\}/', function($match){
                    $type = ($match[1] !== '' && $match[1] !== 's') ? $match[1] : '*';
                    $this->params[] = [
                        "name" => $match[2],
                        "type" => $type
                    ];
        ...
        public function setTargets($targets) {
            ...
            if(!is_array($targets)){
                if(is_callable($targets)){
                    ...
                    throw new InvalidArgumentException('Targets must be array of callables');
                }
            }
        ...
        public function execute($method, $path, $request){
            ...
            for($i = 1; $i < count($matchedParams); $i++){
                if($this->params[$i-1]['type'] === 'i'){
                    $params[$this->params[$i-1]['name']] = intval($matchedParams[$i]);
                }else if($this->params[$i-1]['type'] === 'n'){
                    $params[$this->params[$i-1]['name']] = doubleval($matchedParams[$i]);
                }else{
